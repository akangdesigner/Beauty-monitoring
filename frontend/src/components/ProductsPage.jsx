...
export default function ProductsPage({ isOnline, toast }) {
  ...
  async function load() {
    ...
    try {
      const [prods, brands] = await Promise.all([
        apiFetch('/api/my-products').catch(() => []),
        api.getOwnBrands().catch(() => []),
      ])
      setProducts(prods || [])
      setOwnBrands(brands || [])
    } catch {}
  ...
  async function handleAddBrand() {
    ...
    if (!b) return
    if (ownBrands.includes(b)) { toast('品牌已存在', 'error'); return }
    const next = [...ownBrands, b]
    setBrandSaving(true)
    try {
      await api.setOwnBrands(next)
      setOwnBrands(next)
      setNewBrand('')
  ...
  async function handleRemoveBrand(brand) {
    const next = ownBrands.filter(b => b !== brand)
    try {
      await api.setOwnBrands(next)
      setOwnBrands(next)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="card" style={{ padding: '16px 20px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
          {ownBrands.length === 0 && (
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>尚未設定品牌，請在下方輸入後新增</span>
          )}
          {ownBrands.map(b => (
            <span key={b} style={{
              display: 'inline-flex', alignItems: 'center', gap: 5,
              background: 'rgba(155,109,202,0.2)', border: '1px solid rgba(155,109,202,0.4)',
              color: '#c084fc', borderRadius: 20, padding: '4px 12px', fontSize: 12,
            }}>
              {b}
              <button
                onClick={() => handleRemoveBrand(b)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#c084fc', fontSize: 13, padding: 0, lineHeight: 1, opacity: 0.7 }}
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <select className="select-styled" value={brandFilter} onChange={e => setBrandFilter(e.target.value)}>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px 28px', width: 420, maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 20 }}>{editId ? '編輯商品' : '新增商品'}</div>

            {/* 商品名稱 */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>商品名稱 *</div>
              <input type="text" className="form-input" placeholder="例：水光保濕精華 50ml" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            </div>

            {/* 品牌（下拉 + 自行輸入） */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>品牌</div>
              {ownBrands.length > 0 ? (
                <select
                  className="select-styled"
                  style={{ width: '100%' }}
                  value={ownBrands.includes(form.brand) ? form.brand : '__custom__'}
                  onChange={e => {
                    if (e.target.value === '__custom__') setForm(f => ({ ...f, brand: '' }))
              ...
              {(ownBrands.length === 0 || !ownBrands.includes(form.brand)) && (
                <input
                  type="text"
                  className="form-input"
                  placeholder={ownBrands.length > 0 ? '輸入其他品牌名稱' : '例：LANEIGE'}
                  value={form.brand}
                  onChange={e => setForm(f => ({ ...f, brand: e.target.value }))}
                  style={ownBrands.length > 0 ? { marginTop: 8 } : {}}
                />
              )}
            </div>
            ...
            {/* 售價 */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>售價（NT$）</div>
              <input type="number" className="form-input" placeholder="例：3960" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} />
            </div>

            {/* 圖片 */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>圖片網址</div>
              <input type="url" className="form-input" placeholder="https://…（選填）" value={form.image_url} onChange={e => setForm(f => ({ ...f, image_url: e.target.value }))} />
            </div>

            {/* 備註 */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>備註</div>
              <input type="text" className="form-input" placeholder="選填" value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} />
            </div>
